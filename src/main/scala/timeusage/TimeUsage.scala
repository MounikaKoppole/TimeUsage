...
object TimeUsage {
  ...
  */
  ...
  /** @return
    ...
    *   type String and all the others have type Double. None of the fields are
    *   nullable.
    * @param columnNames
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val head =
      StructField(columnNames.head, DataTypes.StringType, nullable = false)
    val tail = columnNames.tail.map(
      StructField(_, DataTypes.DoubleType, nullable = false)
    )
  ...
  /** @return
    *   a pair (cn, df) where df is a DataFrame and cn holds the column names of
    *   df.
    */
  ...
  /** @return
    ...
    *   hours spent on primary needs are summed together in a single column
    *   (and same for work and leisure). The "teage" column is also projected
    *   to three values: "young", "active", "elder".
    *
    ...
    *
    * This methods builds an intermediate DataFrame that sums up all the
    * columns of each group of activity into a single column.
    *
    ...
    *
    * Note that the initial DataFrame contains time in ''minutes''. You have
  ...
  def timeUsageSummary(primaryNeedsColumns: List[Column], workColumns: List[Column], otherColumns: List[Column], df: DataFrame): DataFrame = {
    ...
    val ageProjection: Column = when(
      $"teage" between (15, 22),
      "young"
    ) when ($"teage" between (23, 55), "active") otherwise "elder" as "age"
  ...
  /** @return
    ...
    *   leisure, grouped by the different ages of life (young, active or
    *   elder), sex and working status.
    * @param summed
  ...
  /** @return
    ...
    * dataset contains one element per group (whose time spent on each
    * activity kind has been aggregated).
    *
